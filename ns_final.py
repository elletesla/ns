...
Original file is located at
    https://colab.research.google.com/drive/1RP9OP8JMFVWTRbL2w_jBVurgq7hZwGWv
"""
...
def load_graph_auto(path):
    ...
    #         raise ValueError("CSV must have at least two columns (source, target).")
        ...
        if df.shape[0] == df.shape[1] and (df.dtypes == int).all():
            try:
...
def plot_degree_distribution(G, bins=30, title="Degree Distribution", show_cumulative=False):
    ...
    if show_cumulative:
        ...
        for p in probs:
            cumulative += p
            ccdf.append(1 - cumulative + p)  # P(K >= k)
...
#IF TEHY ASK : FOR A GIVEN FILE, LOAD IT AND FIND NODES AND EDGES (GRAPH) AND GENRATE ER GRAPH / SCALE FREE FOR THE GIVEN DATA

''''' # --------------------------
# Example: Facebook dataset
# --------------------------
...
if os.path.exists(sample_path):
    ...
    plt.title("Erdős–Rényi Graph (G(n, p))")
    plt.show()

    # --- Scale-Free Graph (power-law) ---
    gamma = 2.5  # exponent for power-law degree distribution
    sf_graph = generate_powerlaw_configuration(n, gamma)

    plt.figure(figsize=(6, 6))
    nx.draw(sf_graph, node_size=10)
    plt.title(f"Scale-Free Graph (Power-law, gamma={gamma})")
    plt.show()
'''
# -----------------------
